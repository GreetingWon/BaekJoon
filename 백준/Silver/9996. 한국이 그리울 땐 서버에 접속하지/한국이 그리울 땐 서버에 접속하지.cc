...
int main() {
	int n; cin >> n; //파일 개수 n
	string pattern; cin >> pattern; //pattern 입력
	int index = pattern.find('*');//별을 기준으로 파트 나누기, 인덱스 번호 받아오기
	
	string part1 = pattern.substr(0, index); //왼쪽 파트 부분 
	string part2 = pattern.substr(index + 1); //오른쪽 파트 부분
